"""
Start FastAPI server WITHOUT scheduler (for testing/development)
...
from models import Team, Player, Game, Injury, Odds, StatsFeatures
from services.chat_ai_v3 import ChatAI
from config import settings

# Create FastAPI app
app = FastAPI(
    title="NFL Betting AI - API Only",
...
async def startup():
    ...
    init_db()
    print("Ready! All NFL data comes from YOUR database.")
...
async def root():
    return {
        ...
        "version": "2.0.0",
        "architecture": "YOUR_AI_BRAIN_GEMINI_MOUTH"
    }
...
async def get_upcoming_games(db: Session = Depends(get_db)):
    ...
    for game in upcoming:
        ...
        games.append({
            ...
            "venue": game.venue,
            "week": game.week,
            "event": game_event  # Special event label for this game
        })
    ...
    return {
        "games": games, 
        "count": len(games),
        "title": title,  # Dynamic title based on context
        "special_event": special_event,  # e.g., "Thanksgiving", "Christmas", or None
...
async def get_trending_players(db: Session = Depends(get_db)):
    ...
    # Get players with high recent performance - eager load Player and Team
    ...
    for f in features:
        if f.player:
            ...
            if f.avg_passing_yards and f.last_3_avg_yards:
                if f.last_3_avg_yards > f.avg_passing_yards * 1.15:
                    insights.append({
                        "type": "hot_streak",
                        "color": "green",
                        "title": "Hot Streak",
            ...
            # Consistency for value bets
            if f.consistency_score and f.consistency_score > 75:
                if f.avg_receiving_yards and f.avg_receiving_yards > 60:
                    insights.append({
                        "type": "value",
                        "color": "gold",
                        "title": "Consistent Target",
                        "message": f"{f.player.name} hitting {f.consistency_score:.0f}% on {f.avg_receiving_yards:.0f}+ rec yds",
                        "player_id": f.player_id
                    })
    
    # Key injuries - eager load Player and Team
